...
void HMDDeviceFactory::processDisplayAtom(RROutput outputXID,
        ...
        EnumerateVisitor &visitor)
{
    ...
    XRRGetOutputProperty (dpy, outputXID, atom,
            0, 100, False, False,
            AnyPropertyType,
            &actual_type, &actual_format,
            &nitems, &bytes_after, &prop);
    propinfo = XRRQueryOutputProperty(dpy, outputXID,
            atom);
    ...
    int mx = 0;
    int my = 0;
    int mwidth = outputInfo->mm_width;
    int mheight = outputInfo->mm_height;

    const uint8_t *val = prop;

    unsigned short expectedVendor = 16082;
    unsigned short vendor = val[8] << 8 | val[9];

    unsigned short expectedCode = 1;
    unsigned short product;
    memcpy(&product, &val[10], 2);
    ...
    if (expectedVendor == vendor &&
            expectedCode == product) {
        HMDDeviceCreateDesc hmdCreateDesc(this,
                idstring, outputInfo->name);

        if(hmdCreateDesc.Is7Inch())
        {
            hmdCreateDesc.SetScreenParameters(mx, my,
        ...
        LogText("OVR::Linux::HMDDevice Initialized hmd %s\n", idstring);
...
void HMDDeviceFactory::checkHMDDevice(Display *dpy, EnumerateVisitor &visitor)
{
    Screen *scr = XDefaultScreenOfDisplay(dpy);
    XRRScreenResources  *res;
    res = XRRGetScreenResourcesCurrent(dpy, scr->root);
    for(int o = 0; o < res->noutput; o++) {
        RROutput outputXID = res->outputs[o];
        XRROutputInfo *outputInfo = XRRGetOutputInfo(dpy, res,
                outputXID);

        int nprop;
        Atom *props;
        props = XRRListOutputProperties(dpy, outputXID,
                &nprop);

        for (int j = 0; j < nprop; j++) {
            processDisplayAtom(outputXID,
                    outputInfo, props[j],
                    dpy, visitor);
        }
